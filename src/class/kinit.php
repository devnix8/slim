...
/**
 * Validates username and password combinations with local Kerberos's 'kinit' executable (requires the local Kerberos installation to be configured correctly).
 * Has been tested on cs.joensuu.fi and it authenticates successfully against Itä-Suomen Yliopisto UEFAD Active Directory.
 * 
 * @author Gima
 */
class KInit {
	 ...
	 * --------------
	 * 
	 * Since 'kinit' insists on writing the ticket to a file, it would be a security vulnerability if the filesystem permissions allowed others access to the file. And since the ticket is not needed (we are just checking login credentials), it's written to '/dev/null'.
	 *
	 * The implementation then checks kinit's return messages to determine whether the authentication attempt was successful or not.
	 */
	...
	/** Forces a login attempt to wait for this many milliseconds before returning. Chat with cs.joensuu.fi administrator resulted the need for this to avoids brute-forcing */
	private $executionDelay = 5000;

	/**
	 * Test if the supplied username and password combination is valid using 'kinit' executable.
	...
	static function auth($username, $password) {
		...
		$sleepDiff = ($kinit->executionDelay*1000) - (microtime(true) - $loginStartTime);
		if ($sleepDiff >= 0) {
			usleep($sleepDiff);
		}
	...
	private function _auth($username, $password) {
		...
		/*
		 * start the 'kinit' process and direct it's stdin, stdout and stderr to php's side
		...
		/*
		 * set the streams to non-blocking mode, so we don't end up in a never-ending function call when reading, such as when no data is available and the end of stream hasn't been reached
		 */ 
		 ...
		 * (stderr): kinit(v5): Credentials cache file permissions incorrect when initializing cache /dev/null
	...
	/**
	 * Read data from two streams seperatedly until 'kinit' has died or timeout exceeded.
	 * 
	 * @param resource $stream1 - Stream 1 to read data from
	 * @param resource $stream2 - Stream 2 to read data from
	 * @param int $timeout - Timeout in milliseconds.
	 * @return mixed - Failure and timeout: false, on success: array ([0] = data from stream 1, [1] = data from stream 2).
	 */
